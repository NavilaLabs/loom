...
pub async fn create(workspace_id: &str, name: String) -> Result<TagRow> {
    ...
    let mut root = Root::<Tag>::record_new(
        TagEvent::Created { id: id.clone(), name: name.clone() }.into(),
    )?;
    repo.save(&mut root).await?;
    Ok(TagRow { id: id.to_string(), name })
}
