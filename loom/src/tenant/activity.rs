...
use loom_core::tenant::{activity::{Activity, ActivityEvent, ActivityId}, project::ProjectId};
...
pub async fn create(
    ...
    let pid: Option<ProjectId> = project_id
        .as_deref()
        .map(|s| s.parse())
        .transpose()?;
    let mut root = Root::<Activity>::record_new(
        ActivityEvent::Created { id: id.clone(), project_id: pid.clone(), name: name.clone() }
            .into(),
    )?;
...
pub async fn update(
    ...
    root.record_that(ActivityEvent::Updated { name, comment, visible, billable }.into())?;
    repo.save(&mut root).await?;
    Ok(())
}
